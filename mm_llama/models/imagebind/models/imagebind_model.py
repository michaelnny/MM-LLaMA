...
from mm_llama.models.imagebind.models.helpers import EinOpsRearrange, LearnableLogitScaling, Normalize, SelectElement, SelectEOSAndProject
from mm_llama.models.imagebind.models.multimodal_preprocessors import (
    AudioPreprocessor,
    IMUPreprocessor,
    PadIm2Video,
    PatchEmbedGeneric,
...
from mm_llama.models.imagebind.models.transformer import MultiheadAttention, SimpleTransformer

ModalityType = SimpleNamespace(
    VISION='vision',
    # TEXT="text", # we don't need text modality from ImageBind
    AUDIO='audio',
    THERMAL='thermal',
    DEPTH='depth',
    IMU='imu',
...
def imagebind_huge(ckpt_path):
    if not ckpt_path or not os.path.exists(ckpt_path):
        raise ValueError(f'Invalid ImageBindModel checkpoint path {ckpt_path!r}')
    ...
    model = ImageBindModel(
        ...
        text_num_heads=16,
        out_embed_dim=1024,
        audio_drop_path=0.0,  # we don't use audio and imu modalities
        imu_drop_path=0.0,
    )

    # Set strict=False since we don't use text modal in the original ImageBind model
