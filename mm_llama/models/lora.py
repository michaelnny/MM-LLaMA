...
def mark_only_lora_as_trainable(model: nn.Module, train_bias: str = 'none', additional_layers: List[str] = []) -> None:
    ...
    for n, p in model.named_parameters():
        if any((l_n in n for l_n in additional_layers)):
...
def lora_state_dict(model: nn.Module, train_bias: str = 'none', additional_layers: List[str] = []) -> Dict[str, torch.Tensor]:
...
def lora_state_dict_from_full_state_dict(state_dict: dict, train_bias: str = 'none', additional_layers: List[str] = []) -> Dict[str, torch.Tensor]:
    """Return state_dict with weights of LoRA's A and B matrices and with biases depending on the `bias` value.
    ...
    if train_bias == 'none':
        return {k: state_dict[k] for k in state_dict if 'lora_' in k or any((l_n in k for l_n in additional_layers))}
    elif train_bias == 'all':
        return {k: state_dict[k] for k in state_dict if 'lora_' in k or 'bias' in k or any((l_n in k for l_n in additional_layers))}
    elif train_bias == 'lora_only':
        to_return = {}
        for k in state_dict:
            if 'lora_' in k:
            ...
            elif any((l_n in k for l_n in additional_layers)):
                to_return[k] = state_dict[k]
